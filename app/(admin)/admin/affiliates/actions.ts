...
export async function createAffiliate(name: string, code: string): Promise<{ error?: string }> {
  ...
  const { data, error } = await admin
    .from('affiliates')
    .insert({ name: trimmedName, code: normalizedCode })
    .select('id')
    .single();
  if (error) {
  ...
  await logAdminAction({
    adminUserId: admin_.userId,
    adminName: admin_.name,
    action: 'affiliate.created',
    targetType: 'affiliate',
    targetId: data.id,
    diff: { name: trimmedName, code: normalizedCode },
  });
